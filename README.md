...
- refuses to dial loopback, link-local, unspecified, cloud metadata addresses, the addresses of the runtime's own host, or a unix socket path. It patches `dns.lookup`, `fetch`, `net.connect`, and `net.Socket.prototype.connect`, so a name is judged on the address it resolves to as the connection is made and each redirect is judged again
...
Level `node` requires Node.js 22.13+, 23.5+, or 24+. At startup the daemon writes the egress rules into its data directory, checks the version of the `node` on `PATH`, then launches `node` once with the same preparation a runtime gets (environment, `NODE_OPTIONS` including the rules, temp directory, and process group), using a temporary directory in place of the service and instance directories. The daemon refuses to start if either check fails. A version-manager shim that needs variables outside the allowlist (for example Volta or asdf) fails this check; put a real `node` binary first on `PATH`. The check runs only at startup, so restart the daemon after changing the `node` on `PATH`. On Node.js 25+, whose permission model blocks network access by default, the daemon grants it with `--allow-net`. Services that start child processes, use worker threads, or load native addons do not work at this level.
