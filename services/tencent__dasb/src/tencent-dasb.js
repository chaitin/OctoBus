...
export const SERVICE_PACKAGE = 'Tencent_DASB.Tencent_DASB';
export const DEFAULT_ENDPOINT = 'https://dasb.tencentcloudapi.com';
export const DEFAULT_VERSION = '2019-10-18';
export const DEFAULT_TIMEOUT_MS = 5000;
export const TENCENT_SERVICE = 'dasb';
...
const validateReadOnlyAction = (action) => {
  ...
  if (!/^(Describe|Search)[A-Za-z0-9]+$/.test(value)) {
    throw errorWithCode('INVALID_ARGUMENT', 'only Tencent Cloud DASB Describe* and Search* actions are allowed');
  }
  return value;
};
...
const invokeTencentCloud = async (action, payload, ctx = {}) => {
  ...
  try {
    ...
    throw errorWithCode('UNAVAILABLE', `failed to call Tencent Cloud DASB API: ${err.message}`);
  }

  const body = await parseTencentResponse(res);
  if (res.status < 200 || res.status >= 300) {
    throw errorWithCode('UNAVAILABLE', `Tencent Cloud DASB API HTTP ${res.status}: ${JSON.stringify(body).slice(0, 500)}`);
  }
