...
import { GrpcError, createTlsDispatcher, fetchWithTimeout, redactSensitive, serviceError } from '@chaitin-ai/octobus-sdk';
...
const errorWithCode = (code, message, details) => serviceError(code, String(redactSensitive(message)), details);
...
const buildTlsOptions = (bindings = {}) => {
  ...
  const dispatcher = createTlsDispatcher(enabled);
...
const throwForHttpStatus = (status) => {
  if (status === 401) throw errorWithCode('UNAUTHENTICATED', 'upstream authentication failed');
  if (status === 403) throw errorWithCode('PERMISSION_DENIED', 'upstream authorization failed');
  if (status >= 400 && status < 500) throw errorWithCode('FAILED_PRECONDITION', `upstream rejected request (HTTP ${status})`);
  throw errorWithCode('UNAVAILABLE', `upstream request failed (HTTP ${status})`);
};
...
const requirePage = (value) => {
  const page = requirePositiveInt(value, 'page');
  if (page > 10000) throw errorWithCode('OUT_OF_RANGE', 'page must not exceed 10000');
  return page;
};

const requirePageSize = (value) => {
  const count = requirePositiveInt(value, 'count');
  if (![10, 20, 50, 100].includes(count)) throw errorWithCode('INVALID_ARGUMENT', 'count must be one of 10, 20, 50, or 100');
  return count;
};
...
const logFlow = (ctx = {}, action, details = {}) => {
  ...
  // Never log credentials, bearer tokens, headers, URLs, or upstream bodies.
...
const handleLogin = async (ctx = {}) => {
  ...
  const { json } = await fetchJson(callCtx, `${callCtx.host}${LOGIN_PATH}`, {
    method: 'POST',
    headers: { 'content-type': 'application/json; charset=utf-8' },
    body: JSON.stringify({ username: callCtx.user, password: callCtx.password }),
  });
  const token = pickFirstString([
    pickStringFrom(json?.token || {}, ['access_token', 'accessToken']),
    json?.token,
    json?.access_token,
    json?.accessToken,
  ]);
  if (hasOwn(json, 'error') || !isSemanticSuccess(json) || !token) {
    logFlow(callCtx, 'Login', { elapsed_ms: Date.now() - started, success: false });
    throw errorWithCode('FAILED_PRECONDITION', '用户登录失败');
  }
  logFlow(callCtx, 'Login', { elapsed_ms: Date.now() - started, success: true });
  return { token };
};

const handleLogout = async (ctx = {}, token) => {
  const callCtx = requireBindings(ctx);
  await fetchRaw(callCtx, `${callCtx.host}${LOGOUT_PATH}`, {
    method: 'POST',
    headers: { 'content-type': 'application/json; charset=utf-8', authorization: `Bearer ${token}` },
  });
  logFlow(callCtx, 'Logout', { success: true });
};
...
const withSession = async (ctx = {}, actionFn) => {
  ...
  let result;
  try {
    result = await actionFn(login.token);
    return result;
  } finally {
    try {
      await handleLogout(ctx, login.token);
    } catch (logoutErr) {
      logFlow(resolveCallContext(ctx), 'Logout', { success: false, error: 'logout request failed' });
    }
  }
};
...
const runQuery = async (req, ctx, action, path, buildPayload) => {
  ...
  return withSession(bound, (token) =>
    callBusiness(bound, token, path, payload).then(({ json, text }) => {
      const out = toQueryResult(json, text);
      logFlow(bound, action, { success: out.success, total: out.total });
...
const runMutation = async (req, ctx, action, path, buildPayload) => {
  ...
  return withSession(bound, (token) =>
    callBusiness(bound, token, path, payload).then(({ json, text }) => {
      ...
      logFlow(bound, action, { success: out.success, id: out.id });
      if (!out.success) throw errorWithCode('FAILED_PRECONDITION', String(out.msg || `${action} failed`));
      return out;
    }));
};
...
const handlerFor = (runner) => (ctx = {}) => runner(requestFromContext(ctx), ctx);
...
export const handlers = {
  [METHOD_QUERY_ATTACK_LOG_FULL]: handlerFor(runQueryAttackLog),
  [METHOD_QUERY_IPLIST_FULL]: handlerFor(runQueryIPList),
  [METHOD_QUERY_INTEL_FULL]: handlerFor(runQueryThreatIntel),
  [METHOD_ADD_INTEL_FULL]: handlerFor(runAddThreatIntel),
  [METHOD_DELETE_INTEL_FULL]: handlerFor(runDeleteThreatIntel),
};
