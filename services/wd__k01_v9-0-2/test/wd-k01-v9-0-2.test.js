...
test('query attack log / ip list / threat intel against mock', async () => {
  ...
  try {
    ...
    assert.equal('login_raw_json' in atk, false);
    assert.equal('logout_raw_text' in atk, false);
    assert.equal(mock.state.tokens.size, 0); // logged out

    const ipl = await invoke(METHOD_QUERY_IPLIST_FULL, { color: 0, dir: 2, ip_search: '1.1.11.0/24' }, ctx);
...
test('add then query then delete threat intel', async () => {
  ...
  try {
    const ctx = buildCtx(mock);
    const add = await invoke(METHOD_ADD_INTEL_FULL, { ip: '192.168.68.69', type: 214, severity: 1 }, ctx);
    assert.equal(add.success, true);
    assert.equal(add.id, 1);
    assert.equal(mock.state.intel.size, 1);

    const q = await invoke(METHOD_QUERY_INTEL_FULL, { source_id: 63 }, ctx);
    assert.equal(q.total, 1);

    const del = await invoke(METHOD_DELETE_INTEL_FULL, { id: 1 }, ctx);
...
test('binding and argument validation', async () => {
  const mock = await createMockServer();
  try {
    await assert.rejects(() => invoke(METHOD_QUERY_ATTACK_LOG_FULL, {}, buildCtx(mock, { bindings: { host: '' } })),
      (e) => e.legacyCode === 'INVALID_ARGUMENT');
    await assert.rejects(() => invoke(METHOD_QUERY_ATTACK_LOG_FULL, {}, buildCtx(mock, { bindings: { user: '', username: '' } })),
      (e) => e.legacyCode === 'INVALID_ARGUMENT');
    await assert.rejects(() => invoke(METHOD_QUERY_ATTACK_LOG_FULL, {}, buildCtx(mock, { bindings: { password: '' } })),
      (e) => e.legacyCode === 'INVALID_ARGUMENT');

    const ctx = buildCtx(mock);
    await assert.rejects(() => invoke(METHOD_ADD_INTEL_FULL, { ip: 'not-ip', type: 1, severity: 1 }, ctx), (e) => e.legacyCode === 'INVALID_ARGUMENT');
    await assert.rejects(() => invoke(METHOD_ADD_INTEL_FULL, { ip: '1.2.3.4', type: 0, severity: 1 }, ctx), (e) => e.legacyCode === 'INVALID_ARGUMENT');
    await assert.rejects(() => invoke(METHOD_DELETE_INTEL_FULL, { id: 0 }, ctx), (e) => e.legacyCode === 'INVALID_ARGUMENT');
  } finally {
    await mock.close();
  }
});
...
test('SDK transport rejects redirects and response values redact credentials', async () => {
  ...
  withFetch(async (_url, init) => {
    request = init;
    return fakeResponse(200, JSON.stringify({ success: true, token: 'must-not-leak', data: { total: 0 } }));
  });
  const out = await _test.fetchJson(buildCtx({ host: 'https://k01:443', user: 'u', password: 'p' }), 'https://k01:443/test');
  assert.equal(request.redirect, 'error');
  assert.equal(out.json.token, 'must-not-leak');
  assert.doesNotMatch(_test.toQueryResult(out.json, out.text).raw_json, /must-not-leak/);
});
...
test('login failure → FAILED_PRECONDITION', async () => {
  ...
  await assert.rejects(() => invoke(METHOD_QUERY_INTEL_FULL, {}, buildCtx({ host: 'https://k01:443', user: 'u', password: 'p' })),
    (e) => e.legacyCode === 'FAILED_PRECONDITION');
});

test('business 401 → UNAUTHENTICATED, and logout still attempted', async () => {
  ...
  await assert.rejects(() => invoke(METHOD_QUERY_ATTACK_LOG_FULL, {}, buildCtx({ host: 'https://k01:443', user: 'u', password: 'p' })),
    (e) => e.legacyCode === 'UNAUTHENTICATED');
...
test('network error → UNAVAILABLE; empty/invalid body → UNKNOWN', async () => {
  ...
  await assert.rejects(() => invoke(METHOD_QUERY_INTEL_FULL, {}, ctx), (e) => e.legacyCode === 'UNAVAILABLE');
  withFetch(async () => fakeResponse(200, '   '));
  await assert.rejects(() => invoke(METHOD_QUERY_INTEL_FULL, {}, ctx), (e) => e.legacyCode === 'UNKNOWN');
...
test('logout failure is swallowed (success path still returns)', async () => {
  ...
  const out = await invoke(METHOD_QUERY_IPLIST_FULL, { color: 1, dir: 0 }, buildCtx({ host: 'https://k01:443', user: 'u', password: 'p' }));
  assert.equal(out.success, true);
  assert.equal('logout_raw_text' in out, false);
});
...
test('helper coverage', () => {
  ...
  assert.equal(h.resolveHost({ host: 'k01.local', restBaseUrl: 'https://k01:443/' }), 'https://k01:443');
...
test('fetch error fallback message', async () => {
  ...
  await assert.rejects(() => _test.fetchRaw({ bindings: {}, limits: {} }, 'http://x', {}),
    (e) => e.legacyCode === 'UNAVAILABLE' && e.message === 'm');
});
...
test('camelCase meta is logged without error', async () => {
  ...
  try {
    ...
    const out = await invoke(METHOD_QUERY_INTEL_FULL, {}, ctx);
    assert.equal(out.success, true);
  } finally {
    await mock.close();
  }
...
test('business failure with logout also failing still rejects with business error', async () => {
  ...
  await assert.rejects(() => invoke(METHOD_DELETE_INTEL_FULL, { id: 9 }, buildCtx({ host: 'https://k01:443', user: 'u', password: 'p' })),
