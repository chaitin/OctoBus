...
const resolveTimeoutMs = (ctx = {}) => boundedPositiveInt(
  firstDefined(ctx.limits?.timeoutMs, ctx.bindings?.timeoutMs),
  DEFAULT_TIMEOUT_MS,
  MAX_TIMEOUT_MS,
);
const resolveMaxResponseBytes = (ctx = {}) => boundedPositiveInt(
  firstDefined(ctx.limits?.maxResponseBytes, ctx.bindings?.maxResponseBytes),
...
const validateBindings = (bindings = {}) => {
  ...
  if (url.username || url.password) throw errorWithCode('FAILED_PRECONDITION', 'binding "endpoint" must not contain credentials');
  if (url.pathname !== '/' || url.search || url.hash) throw errorWithCode('FAILED_PRECONDITION', 'binding "endpoint" must not contain a path, query, or fragment');
  ...
  return {
    ...
    skipTlsVerify: toBool(firstDefined(bindings.skipTlsVerify, bindings.tlsInsecureSkipVerify)),
  };
};
...
const parseTencentResponse = async (res, maxResponseBytes) => {
  const declaredLength = Number(res.headers?.get?.('content-length') || 0);
  if (Number.isFinite(declaredLength) && declaredLength > maxResponseBytes) {
    throw errorWithCode('RESOURCE_EXHAUSTED', `Tencent Cloud API response exceeds ${maxResponseBytes} bytes`);
  }
...
const invokeTencentCloud = async (action, payload, ctx = {}) => {
  ...
  try {
    res = await fetch(bindings.endpoint, {
      ...
      redirect: 'error',
      ...(bindings.skipTlsVerify && bindings.endpoint.startsWith('https:')
        ? { dispatcher: (insecureTlsDispatcher ??= new Agent({ connect: { rejectUnauthorized: false } })) }
        : {}),
    });
    body = await parseTencentResponse(res, maxResponseBytes);
  } catch (err) {
    ...
    throw errorWithCode('UNAVAILABLE', 'failed to call Tencent Cloud CSIP API');
  } finally {
