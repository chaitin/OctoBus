...
test('validates required Tencent Cloud credentials and endpoint aliases', () => {
  ...
  assert.throws(() => _test.validateBindings({ endpoint: 'https://user:pass@example.com', secretId: 'id', secretKey: 'key' }), /credentials/);
  assert.throws(() => _test.validateBindings({ endpoint: 'https://example.com/api', secretId: 'id', secretKey: 'key' }), /path/);
  assert.throws(() => _test.validateBindings({ endpoint: 'not a url', secretId: 'id', secretKey: 'key' }), /valid URL/);
  assert.equal(_test.validateBindings({ endpoint: 'https://example.com', secretId: 'id', secretKey: 'key', skipTlsVerify: true }).skipTlsVerify, true);
  assert.equal(_test.validateBindings({ endpoint: 'https://example.com', secretId: 'id', secretKey: 'key', skipTlsVerify: 'yes' }).skipTlsVerify, true);
  assert.equal(_test.validateBindings({ endpoint: 'https://example.com', secretId: 'id', secretKey: 'key', skipTlsVerify: 0 }).skipTlsVerify, false);
  assert.equal(_test.validateBindings({ endpoint: 'https://example.com', secretId: 'id', secretKey: 'key', skipTlsVerify: 'off' }).skipTlsVerify, false);
  assert.equal(_test.validateBindings({ endpoint: 'https://example.com', secretId: 'id', secretKey: 'key', skipTlsVerify: {} }).skipTlsVerify, false);
  assert.equal(_test.resolveTimeoutMs({ bindings: { timeoutMs: 999999 } }), 120000);
  assert.equal(_test.resolveMaxResponseBytes({ bindings: { maxResponseBytes: 99999999 } }), 10 * 1024 * 1024);
});
...
test('normalizes protobuf Struct payloads', () => {
  ...
  assert.deepEqual(_test.normalizeStruct({ plain: { value: 'wrapped' }, list: [1, { boolValue: false }] }), { plain: 'wrapped', list: [1, false] });
...
test('every public RPC reaches the adapter with its matching Tencent action', async () => {
  const seen = [];
  setFetch(async (_url, init) => {
    seen.push(init.headers['X-TC-Action']);
    return response(200, { Response: { RequestId: `req-${seen.length}` } });
  });

  for (const action of READ_ONLY_ACTIONS) {
    const result = await handlers[`${SERVICE_PACKAGE}/${action}`]({ request: { payload: {} }, ...buildCtx() });
    assert.equal(result.response.structValue.fields.RequestId.stringValue, `req-${seen.length}`);
  }
  assert.deepEqual(seen, READ_ONLY_ACTIONS);
});
...
test('maps Tencent Cloud and transport errors', async () => {
  ...
  setFetch(async () => response(200, { Response: { Error: { Code: 'InvalidParameterValue', Message: 'bad input' } } }));
  await expectGrpcError(() => handlers[`${SERVICE_PACKAGE}/DescribeAlertList`]({}, buildCtx()), 'INVALID_ARGUMENT');

  setFetch(async () => response(200, { Response: { Error: { Code: 'InternalError', Message: 'busy' } } }));
  await expectGrpcError(() => handlers[`${SERVICE_PACKAGE}/DescribeAlertList`]({}, buildCtx()), 'UNKNOWN');
  ...
  setFetch(async () => { throw new Error('https://AKIDEXAMPLE:SECRETEXAMPLE@example.invalid'); });
  await expectGrpcError(
    () => handlers[`${SERVICE_PACKAGE}/DescribeAlertList`]({}, buildCtx()),
    'UNAVAILABLE',
    (err) => {
      assert.doesNotMatch(err.message, /AKIDEXAMPLE|SECRETEXAMPLE/);
      assert.match(err.message, /failed to call/);
    },
  );
});

test('bounds upstream responses before parsing', async () => {
  setFetch(async () => ({
    status: 200,
    headers: { get: () => '100' },
    text: async () => '{"Response":{}}',
  ...
  await expectGrpcError(
    () => handlers[`${SERVICE_PACKAGE}/DescribeAlertList`]({}, buildCtx({ limits: { maxResponseBytes: 10 } })),
    'RESOURCE_EXHAUSTED',
  );

  setFetch(async () => response(200, { Response: { data: '0123456789' } }));
  await expectGrpcError(
    () => handlers[`${SERVICE_PACKAGE}/DescribeAlertList`]({}, buildCtx({ limits: { maxResponseBytes: 5 } })),
    'RESOURCE_EXHAUSTED',
  );
});
