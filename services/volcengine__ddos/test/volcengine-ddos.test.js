...
test('signs and sends POST ddos request with body payload', async () => {
  ...
  const url = new URL(captured.url);
  assert.equal(url.origin, 'https://ddos.cn-beijing.volcengineapi.com');
  assert.equal(url.searchParams.get('Action'), 'GetAlarm');
  assert.equal(url.searchParams.get('Version'), '2020-12-08');
...
test('maps Volcengine and transport errors', async () => {
  ...
  await expectGrpcError(
    () => handlers[`${SERVICE_PACKAGE}/GetBasicAlarm`]({}, buildCtx()),
    'PERMISSION_DENIED',
  ...
  await expectGrpcError(
    () => handlers[`${SERVICE_PACKAGE}/GetBasicAlarm`]({}, buildCtx({ limits: { timeoutMs: 25 } })),
    'DEADLINE_EXCEEDED',
  ...
  await expectGrpcError(
    () => handlers[`${SERVICE_PACKAGE}/GetBasicAlarm`]({}, buildCtx({ limits: { timeoutMs: 5 } })),
    'DEADLINE_EXCEEDED',
